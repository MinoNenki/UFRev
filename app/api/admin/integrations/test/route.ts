...
import { mergeConnectionChecks, type IntegrationTestTarget, validateIntegrationConnections } from '@/lib/integration-health';
...
function normalizeTarget(value: FormDataEntryValue | null): IntegrationTestTarget {
  const raw = String(value || 'all');
  if (raw === 'shopify' || raw === 'woocommerce' || raw === 'social') return raw;
  return 'all';
}
...
export async function POST(req: NextRequest) {
  ...
  const testTarget = normalizeTarget(formData.get('testTarget'));
  ...
  const currentSettings = await getIntegrationSettings();
  const nextChecks = await validateIntegrationConnections({
    shopifyStoreDomain,
    woocommerceStoreUrl,
    tiktokAdAccountId,
    metaBusinessId,
  ...
  await supabaseAdmin.from('app_config').upsert({
    ...
    value_json: {
      ...
      connection_checks: mergeConnectionChecks({
        current: currentSettings.connectionChecks,
        next: nextChecks,
        target: testTarget,
      }),
    },
  ...
  return NextResponse.redirect(new URL(`/admin/integrations?tested=${testTarget}`, req.url));
}