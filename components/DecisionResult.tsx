...
function MoneyRow({
  ...
  return (
    <div className="decision-money-row grid grid-cols-1 items-start gap-1.5 rounded-[20px] border border-white/10 bg-slate-950/40 px-4 py-3.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:gap-3">
      <div className="decision-money-label min-w-0 text-sm leading-5 text-slate-300 break-normal [overflow-wrap:break-word]">{label}</div>
      <div className="decision-money-value text-left text-sm font-semibold text-white sm:pl-2 sm:text-right sm:whitespace-nowrap">
...
function buildDecisionResultViewModel(result: DecisionResultShape, currentLanguage: Language) {
  ...
  const infoCardClass = 'decision-info-card min-w-0 rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(15,23,42,0.42))] p-3 sm:p-5';
  const infoTitleClass = 'text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-300 sm:text-[11px] sm:tracking-[0.18em]';
  const infoBodyClass = 'decision-info-body mt-3 space-y-2 text-[13px] leading-6 text-slate-100 break-normal [overflow-wrap:break-word] sm:text-[15px] sm:leading-7';
    ...
    <div className="space-y-3 sm:space-y-4 lg:space-y-5">
      <div className="rounded-[30px] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.12),transparent_24%),linear-gradient(135deg,rgba(255,255,255,0.04),rgba(15,23,42,0.72))] p-3 sm:p-4 lg:p-5 xl:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="w-full rounded-[24px] border border-white/10 bg-slate-950/40 p-4 sm:p-5 xl:w-auto xl:max-w-[260px]">
      ...
      {result.pricing?.quantityScenarios && (
        <div className="rounded-[24px] border border-cyan-300/20 bg-[linear-gradient(135deg,rgba(34,211,238,0.12),rgba(15,23,42,0.75))] p-4 sm:p-5">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="rounded-full border border-cyan-200/30 bg-slate-950/45 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-cyan-100">
              {tt(currentLanguage, { en: '1-unit test vs 5-unit scale', pl: 'Test 1 sztuki vs skala 5 sztuk' })}
            </div>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {(['one', 'five'] as const).map((key) => {
              ...
              return (
                <div
                  key={key}
                  className={`rounded-[20px] border p-4 ${isOne ? 'border-amber-300/30 bg-slate-950/60' : 'border-white/10 bg-slate-950/45'}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.18em] text-slate-400">
                        {isOne ? tt(currentLanguage, { en: '1 unit', pl: '1 sztuka' }) : tt(currentLanguage, { en: '5 units', pl: '5 sztuk' })}
                      </div>
                      <div className="mt-2 text-2xl font-black text-white">{formatMoney(scenario.recommendedSellTargetUsd, currentLanguage, 'USD')}</div>
                    </div>
                    <div className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] ${isOne ? 'border-amber-300/30 bg-amber-400/10 text-amber-100' : 'border-cyan-300/30 bg-cyan-400/10 text-cyan-100'}`}>
                      {scenario.projectedMarginPercent}%
                    </div>
                  </div>

                  <div className="mt-2 text-sm text-slate-300">
                    {formatMoney(scenario.recommendedSellTargetPln, currentLanguage, 'PLN')} / {tt(currentLanguage, { en: 'PLN target', pl: 'cel PLN' })}
                  </div>

                  <div className="mt-4 space-y-2 text-sm text-slate-200">
                    <div className="flex items-center justify-between gap-3">
                      <span>{tt(currentLanguage, { en: 'Landed cost', pl: 'Koszt lądowy' })}</span>
                      <span className="font-semibold text-white">{formatMoney(scenario.landedCostUsd, currentLanguage, 'USD')}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span>{tt(currentLanguage, { en: 'Gross profit', pl: 'Zysk brutto' })}</span>
                      <span className="font-semibold text-white">{formatMoney(scenario.projectedGrossProfitUsd, currentLanguage, 'USD')}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span>{tt(currentLanguage, { en: 'Margin', pl: 'Marża' })}</span>
                      <span className="font-semibold text-white">{scenario.projectedMarginPercent}%</span>
                    </div>
                  </div>

                  <div className="mt-3 text-[11px] uppercase tracking-[0.16em] text-slate-400">
                    {isOne
                      ? tt(currentLanguage, { en: 'Safer for a first test', pl: 'Bezpieczniejsze przy pierwszym teście' })
                      : tt(currentLanguage, { en: 'Better scale efficiency', pl: 'Lepsza efektywność skali' })}
                  </div>
                </div>
              );
            })}
        ...
        <div className="rounded-[24px] border border-cyan-300/20 bg-cyan-300/10 p-4 sm:p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-3">
            {(hybridBreakdown.lanes || []).map((lane) => (
              <div key={lane.key} className="rounded-2xl border border-white/10 bg-slate-950/45 p-3 sm:p-4">
...
export function AdvancedDecisionReasoning({
  ...
  return (
    <details className="advanced-reasoning-shell advanced-reasoning-hero advanced-reasoning-shell-mobile rounded-[32px] border border-white/10 bg-white/[0.03] p-3 sm:p-5 xl:p-8" open>
      ...
      <div className="advanced-reasoning-main-grid mt-6 grid grid-cols-1 gap-3 lg:grid-cols-2 lg:gap-4">
